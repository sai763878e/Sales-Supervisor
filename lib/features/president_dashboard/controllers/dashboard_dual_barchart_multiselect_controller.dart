...
import 'package:sales_supervisor/features/president_dashboard/models/dashboard_dual_chart_multiselect_model.dart';
...
class DashboardDualBarchartMultiselectController extends GetxController {
  DashboardDualBarchartMultiselectController(
  ...
  final dashboardChartModelMap =
      <String, DashboardDualChartMultiselectModel>{}.obs;
  ...
  parseDashboard(Rx<DashboardComponentModel> model) async {
    try {
      ...
      views.forEach((key, value) {
        DashboardDualChartMultiselectModel
        dashboardDualBarchartMultiselectModel =
        DashboardDualChartMultiselectModel();

        dashboardDualBarchartMultiselectModel.title = value["Title"];
        dashboardDualBarchartMultiselectModel.subTitle = value["Subtitle"];
        dashboardDualBarchartMultiselectModel.key = key;
        ...
        dataMap.forEach((key, value) {
          ...
          dashboardDualBarchartMultiselectModel.chartData
              .addIf(true, key, chartData);
        });

        dashboardChartModelMap.value
            .addIf(true, key, dashboardDualBarchartMultiselectModel);
      });
